                           ...
                           <xsl:if test="j:array[@key='parameters']/*">
                              <xsl:for-each select="j:array[@key='parameters']/j:map">
                                 <xsl:choose>
                                    ...
                                    <!-- Parameter as schema reference -->
                                    <xsl:when test="j:string[@key='in']='query' and j:map[@key='schema']/j:string[@key='$ref']">
                                       <xsl:variable name="splitString" select="tokenize(j:map[@key='schema']/j:string[@key='$ref'], '/')" />
                                       <xsl:variable name="components" select="$splitString[2]" />
                                       <xsl:variable name="schemas" select="$splitString[3]" />
                                       <xsl:variable name="paramName" select="$splitString[4]" />
                                       <xsl:choose>
                                          <xsl:when test="ancestor::j:map/j:map[@key=$components]/j:map[@key=$schemas]/j:map[@key=$paramName]/j:map[@key='properties']/*">
                                             <xsl:for-each select="ancestor::j:map/j:map[@key=$components]/j:map[@key=$schemas]/j:map[@key=$paramName]/j:map[@key='properties']/j:map">
                                                <j:map>
                                                   <j:string key="param">{@key}</j:string>
                                                   <j:string key="value" />
                                                </j:map>
                                             </xsl:for-each>
                                          </xsl:when>
                                          <xsl:otherwise>
                                             <xsl:if test="j:string[@key='in']='query'">
                                                <j:map>
                                    ...
                                    <!-- Parameter as inline -->
                                    <xsl:when test="j:string[@key='in']='query' and not(j:map[@key='schema']/j:string[@key='$ref'])">
                                       <j:map>
                                          <j:string key="param">{j:string[@key='name']}</j:string>
                                          <xsl:choose>
                                             <xsl:when test="number(j:map[@key='schema']/j:number[@key='default'])">
                              ...
                              <xsl:for-each select="j:array[@key='parameters']/j:map">
                                 <xsl:choose>
                                    ...
                                    <xsl:when test="j:string[@key='$ref']">
                                       ...
                                       <xsl:variable name="paramName" select="$splitString[4]" />
                                       <xsl:for-each select="ancestor::j:map/j:map[@key=$components]">
                                          <xsl:if test="j:map[@key=$parameters]/j:map[@key=$paramName]/* and j:map[@key=$parameters]/j:map[@key=$paramName]/j:string[@key='in']='header'">
                                             <j:map>
                                                <j:string key="param">{j:map[@key=$parameters]/j:map[@key=$paramName]/j:string[@key='name']}</j:string>
                                                <xsl:choose>
                                                   <xsl:when test="number(j:map[@key=$parameters]/j:map[@key=$paramName]/j:map[@key='schema']/j:number[@key='default'])">
                                    ...
                                    <!-- Parameter as schema reference -->
                                    <xsl:when test="j:string[@key='in']='header' and j:map[@key='schema']/j:string[@key='$ref']">
                                       <xsl:variable name="splitString" select="tokenize(j:map[@key='schema']/j:string[@key='$ref'], '/')" />
                                       <xsl:variable name="components" select="$splitString[2]" />
                                       <xsl:variable name="schemas" select="$splitString[3]" />
                                       <xsl:variable name="paramName" select="$splitString[4]" />
                                       <xsl:choose>
                                          ...
                                          <xsl:otherwise>
                                             <xsl:if test="j:string[@key='in']='header'">
                                                <j:map>
                                                   <j:string key="param">{j:string[@key='name']}</j:string>
                                                   <xsl:choose>
                                                      <xsl:when test="number(j:map[@key='schema']/j:number[@key='default'])">
                                    ...
                                    <!-- Parameter as inline -->
                                    <xsl:when test="j:string[@key='in']='header' and not(j:map[@key='schema']/j:string[@key='$ref'])">
                                       <j:map>
                                          <j:string key="param">{j:string[@key='name']}</j:string>
                                          <xsl:choose>
                                             <xsl:when test="number(j:map[@key='schema']/j:number[@key='default'])">
